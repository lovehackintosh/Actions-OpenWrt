...
aria2c --stderr -c -x8 -s16 -j16 -k1M --disable-ipv6 --dir=./dl https://cdn.kernel.org/pub/linux/kernel/v5.x/linux-$PATCHVER.tar.xz || aria2c --stderr -c -x8 -s16 -j16 -k1M --disable-ipv6 --dir=./dl https://cdn.kernel.org/pub/linux/kernel/v6.x/linux-$PATCHVER.tar.xz
...
for target in $targets; do
	...
	[ -d target/linux/$target/patches-${KERNEL} ] && {
		...
		if [ ! -f tmp/${target}_${PATCHVER}_refreshed ]; then
			$CMD make target/linux/refresh V=${VERBOSE} KERNEL_PATCHVER=${KERNEL} LINUX_VERSION=${PATCHVER} -j 16 || exit 1
			$CMD make target/linux/prepare KERNEL_PATCHVER=${KERNEL} LINUX_VERSION=${PATCHVER} -j 16 || exit 1
...
if [ "$UPDATE" -eq 1 ]; then
	...
	$CMD sed -i include/kernel-$KERNEL \
